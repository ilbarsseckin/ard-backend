-- ════════════════════════════════════════════════════════════
--  BASKIKAPINDA YAPISI - V2 (FK constraint fix)
--  Önce parent_id'leri NULL'a çek, sonra DELETE
-- ════════════════════════════════════════════════════════════

BEGIN;

-- ─── 0) ÖNCE PARENT_ID TEMİZLİĞİ ──────────────────────
-- Silinecek kategorilere işaret eden child'lar için parent_id'yi NULL yap
UPDATE catalog_categories SET parent_id = NULL
WHERE slug IN ('afis', 'bayrak', 'roll-up');

-- Silinecek olanların altındakileri de NULL'a çek (zincir koruması)
UPDATE catalog_categories SET parent_id = NULL
WHERE parent_id IN (
